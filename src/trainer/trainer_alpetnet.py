# trainer/trainer_alpetnet.py
"""
Trainer module for ALPETNet lip reading model.

This module implements the training and validation pipeline for the ALPETNet architecture,
...
class ALPETNetTrainer:
    """Trainer class for the ALPETNet lip reading model.
    
    This class handles the training loop, validation, metrics calculation,
    and model checkpointing for the ALPETNet architecture. It supports multiple
    ...
    def save_checkpoint(self, loss: float, wer: float, cer: float, bleu: float, train_losses: list, val_losses: list, epochs_list: list):
        ...
        checkpoint_name = f'ALPETNet_{timestamp}_loss_{loss:.4f}_wer_{wer:.4f}_cer_{cer:.4f}_bleu_{bleu:.4f}.pt'
