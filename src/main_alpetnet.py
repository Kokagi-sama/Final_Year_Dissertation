# main_alpetnet.py
"""
Script for training and evaluating ALPETNet lip reading models.

This script loads configuration, prepares datasets, and manages the training pipeline
for the ALPETNet architecture. It handles training loops with periodic validation,
checkpoint management, performance metric tracking, and proper resource cleanup.
The model is designed for lip reading tasks with focus on optimizing Character Error
Rate (CER) and Word Error Rate (WER) metrics.
"""
...
from models.ALPETNet import ALPETNet # ALPETNet model architecture
from dataset_loader.lip_dataset import LipDataset # Dataset for lip reading
from trainer.trainer_alpetnet import ALPETNetTrainer # Trainer for ALPETNet

# Global tracking flag to prevent duplicate cleanup operations
_is_cleaning_up = False

# Store objects globally for cleanup
resources = {}
...
def cleanup_dataloader(dataloader):
    ...
    # First, close the dataloader explicitly 
    try:
        # Stop workers
...
def find_latest_checkpoint(directory: Path) -> tuple[Path, float, float]:
    ...
    timestamp_pattern = re.compile(r"ALPETNet_(\d{8}_\d{6})_") # Format: YYYYMMDD_HHMMSS
    ...
    checkpoints = list(directory.glob("ALPETNet_*.pt"))
